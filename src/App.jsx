...
// Función auxiliar para generar arreglo de imágenes dinámicamente
const generateScreenshots = (id, count, ext = 'png') => {
    return Array.from({ length: count }, (_, i) => `images/screenshotsId${id}/img${i + 1}.${ext}`);
};
...
Enviado desde tbxtechnologies.github.io`;
    ...
    const projects = [
        ...
        {
            id: 2,
            title: "TBX SwimManager Sitio Web",
        ...
        {
            id: 13,
            title: "TBX SwimManager APP",
            image: "images/tbxsmapp.png",
            tag: "App Android",
            category: "app",
            description: "La aplicación permite gestionar nadadores, entrenadores y equipos, así como crear y administrar competencias deportivas. Cuenta con herramientas para la captura de tiempos y resultados en tiempo real por parte de los jueces, visualización de rankings en vivo, un dashboard con estadísticas detalladas y un sistema de acceso seguro basado en roles de usuario.",
            screenshots: generateScreenshots(13, 31, 'jpg') // Genera img1.png hasta img5.png
        },
        {
            id: 14,
            title: "Animación de Concierto ",
            image: "images/logo.png",
            tag: "Animación en 3ds Max",
            category: "3d",
            description: "Modelado y renderizado fotorrealista en 3ds Max para la preventa de un complejo residencial. Se cuidaron detalles como texturas de alta resolución e iluminación global para mayor realismo.",
        ...
        <div className="min-h-screen">
            ...
            <section className="py-12 md:py-16 bg-slate-950">
                <div className="container mx-auto px-4 md:px-6">
                    ...
                    <div className="flex flex-wrap justify-center gap-3 md:gap-4 max-w-6xl mx-auto [&>div]:w-28 md:[&>div]:w-32">
                        {/* React */}
                ...
                <div className="container mx-auto px-4 md:px-6">
                    ...
                    <div className="flex flex-wrap gap-3 md:gap-4 mb-8 md:mb-12 justify-center">
                        <button
                            ...
                            className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold text-xs md:text-sm uppercase tracking-wider transition-all ${portfolioFilter === 'todos'
                        ...
                        <button
                            ...
                            className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold text-xs md:text-sm uppercase tracking-wider transition-all ${portfolioFilter === 'trabajo'
                                ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/25'
                                : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold text-xs md:text-sm uppercase tracking-wider transition-all ${portfolioFilter === 'modelo'
                                ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/25'
                                : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold text-xs md:text-sm uppercase tracking-wider transition-all ${portfolioFilter === 'app'
                                ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/25'
                                : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'
                                }`}
                        >
                        ...
                        <button
                            onClick={() => setPortfolioFilter('3d')}
                            className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-semibold text-xs md:text-sm uppercase tracking-wider transition-all ${portfolioFilter === '3d'
                                ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/25'
                                : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'
                                }`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                        {filteredProjects.map((project) => {
                            const isModalProject = project.category === 'app' || project.category === '3d';
                            const Element = isModalProject ? 'div' : 'a';
                            const elementProps = isModalProject
                                ? { onClick: () => setSelectedProject(project), className: "group relative overflow-hidden rounded-xl cursor-pointer block" }
                                : { href: project.url, target: "_blank", rel: "noopener noreferrer", className: "group relative overflow-hidden rounded-xl cursor-pointer block" };
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-900/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-8">
                                        ...
                                        <div className="mt-4 flex items-center gap-2 text-white/80">
                                            <span>{isModalProject ? 'Ver Detalles' : 'Ver Proyecto'}</span>
            ...
            {selectedProject && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
                        <button
                            onClick={() => setSelectedProject(null)}
                            className="absolute top-4 right-4 p-2 bg-slate-800 hover:bg-cyan-500 text-slate-300 hover:text-white rounded-full transition-colors z-10"
                        >
                            <IconX className="w-5 h-5" />
                        </button>

                        <div className="w-full aspect-video bg-slate-800 relative">
                            <img
                        ...
                        <div className="p-6 md:p-8">
                            ...
                            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">{selectedProject.title}</h2>
                            ...
                            {selectedProject.screenshots && selectedProject.screenshots.length > 0 && (
                                <div className="mb-6">
                                    ...
                                    <div className={`grid gap-4 ${selectedProject.category === 'app' ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3'}`}>
                                        {selectedProject.screenshots.map((img, i) => (
                                            <div key={i} className={`${selectedProject.category === 'app' ? 'aspect-[9/16]' : 'aspect-video'} bg-slate-800 rounded-lg overflow-hidden border border-slate-700`}>
                                                <img src={img} alt={`Captura ${i + 1}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-300" />
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-wrap justify-end gap-3 mt-8">
                                <button
                                    onClick={() => setSelectedProject(null)}
                                    className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-full font-bold transition-colors border border-slate-700 text-sm md:text-base"
                                >
                                    Cerrar Detalles
                                </button>
                            </div>
